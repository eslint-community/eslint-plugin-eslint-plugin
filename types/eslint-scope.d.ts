...
// eslint-disable-next-line n/no-extraneous-import -- only types are needed
import type { Visitor, VisitorOptions } from 'esrecurse';
import * as ESTree from 'estree';

/**
 * Forking `eslint-scope` types due to an incompatibility with built-in `eslint` types.
 * Once this issue is resolve, we can go back to using `eslint-scope` types directly.
 * The main change here is replacing `eslint-scope`'s ScopeManager with `eslint`'s Scope.ScopeManager.
 * https://github.com/eslint/js/issues/719
 */
...
declare module 'eslint-scope' {
   ...
   */
  export type ScopeManager = eslint.Scope.ScopeManager;
  ...
  export class Variable<TReference extends Reference = Reference>
    implements eslint.Scope.Variable
  {
    /**
